/**
 * RiskMatrix.jsx — Section 11: Risk Matrix
 * Single "Generate All" button generates the sole field.
 */

import React, { useCallback } from "react";
import { Sparkles } from "lucide-react";
import LLMFieldBlock from "../components/LLMFieldBlock";
import { RISK_MATRIX_FIELD_DEFS, FIELD_API_PAYLOAD } from "../data/pddStructure";
  ...
  return (
    <div style={{ display: "flex", justifyContent: "flex-end", margin: 0 }}>
      ...
      >
        {isAnyLoading ? (
          <>
            <span style={{
              ...
              borderRadius: "50%", animation: "sec11-spin 0.7s linear infinite",
            }} />
            Generating…
          </>
        ) : (
        ...
        <style>{`@keyframes sec11-spin { to { transform: rotate(360deg); } }`}</style>
      </button>
    </div>
  );
}
  ...
  return (
    <div className="pdd-content-container">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <h1 style={{ margin: 0 }}>11. Risk Matrix</h1>
        <SectionGenerateBtn
          fields={RISK_MATRIX_FIELD_DEFS}
      ...
      {RISK_MATRIX_FIELD_DEFS.map((field) => (
        <LLMFieldBlock
          key={field.id}
          fieldId={field.id}
          label={field.label}
