...
import { ES_SIGNOFF_FIELD_DEFS, FIELD_API_PAYLOAD } from "../data/pddStructure";
  ...
  return (
    <div style={{ display: "flex", justifyContent: "flex-end", margin: 0 }}>
      ...
      >
        {isAnyLoading ? (
          <>
            <span style={{
              ...
              borderRadius: "50%", animation: "sec20-spin 0.7s linear infinite",
            }} />
            Generating…
          </>
        ) : (
        ...
        <style>{`@keyframes sec20-spin { to { transform: rotate(360deg); } }`}</style>
      </button>
    </div>
  );
}

export default function ESSignoff({
  village,
  fieldContent,
  fieldLoading,
  fieldErrors,
  ...
  return (
    <div className="pdd-content-container">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <h1 style={{ margin: 0 }}>20. E&S Screening Sign-off</h1>
        <SectionGenerateBtn
          fields={ES_SIGNOFF_FIELD_DEFS}
      ...
      {ES_SIGNOFF_FIELD_DEFS.map((field) => (
        <LLMFieldBlock
          key={field.id}
          fieldId={field.id}
          label={field.label}
