/**
 * ProjectImplementation.jsx — Section 15: Project Implementation
 */

import React, { useCallback } from "react";
import { Sparkles } from "lucide-react";
import LLMFieldBlock from "../components/LLMFieldBlock";
import { IMPLEMENTATION_FIELD_DEFS, FIELD_API_PAYLOAD } from "../data/pddStructure";
  ...
  return (
    <div style={{ display: "flex", justifyContent: "flex-end", margin: 0 }}>
      ...
      >
        {isAnyLoading ? (
          <>
            <span style={{
              ...
              borderRadius: "50%", animation: "sec15-spin 0.7s linear infinite",
            }} />
            Generating…
          </>
        ) : (
        ...
        <style>{`@keyframes sec15-spin { to { transform: rotate(360deg); } }`}</style>
      </button>
    </div>
  );
}
  ...
  return (
    <div className="pdd-content-container">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <h1 style={{ margin: 0 }}>15. Project Implementation</h1>
        <SectionGenerateBtn
          fields={IMPLEMENTATION_FIELD_DEFS}
      ...
      {IMPLEMENTATION_FIELD_DEFS.map((field) => (
        <LLMFieldBlock
          key={field.id}
          fieldId={field.id}
          label={field.label}
