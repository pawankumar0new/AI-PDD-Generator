/**
 * QuantitiesCostEstimates.jsx — Section 12: Quantities and Cost Estimates
 */

import React, { useCallback } from "react";
import { Sparkles } from "lucide-react";
import LLMFieldBlock from "../components/LLMFieldBlock";
import { COST_ESTIMATES_FIELD_DEFS, FIELD_API_PAYLOAD } from "../data/pddStructure";
  ...
  return (
    <div style={{ display: "flex", justifyContent: "flex-end", margin: 0 }}>
      ...
      >
        {isAnyLoading ? (
          <>
            <span style={{
              ...
              borderRadius: "50%", animation: "sec12-spin 0.7s linear infinite",
            }} />
            Generating…
          </>
        ) : (
        ...
        <style>{`@keyframes sec12-spin { to { transform: rotate(360deg); } }`}</style>
      </button>
    </div>
  );
}

export default function QuantitiesCostEstimates({
  village,
  fieldContent,
  fieldLoading,
  fieldErrors,
  ...
  return (
    <div className="pdd-content-container">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <h1 style={{ margin: 0 }}>12. Quantities and Cost Estimates</h1>
        <SectionGenerateBtn
          fields={COST_ESTIMATES_FIELD_DEFS}
      ...
      {COST_ESTIMATES_FIELD_DEFS.map((field) => (
        <LLMFieldBlock
          key={field.id}
          fieldId={field.id}
          label={field.label}
